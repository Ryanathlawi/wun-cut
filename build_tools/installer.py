...
PUBLISHER = "Athlawi"
VERSION = "1.0"
TAGLINE = "محرّر فيديو عربيّ · مجّاني · بلا إنترنت"
COPYRIGHT = "© 2026 Athlawi"
REG_KEY = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\WunCut"
EXE_NAME = "Wun Cut.exe"
PAYLOAD = "payload.zip"
...
def wipe(folder, keep=()):
    """
    يحذف محتوى المجلد الآن، ويرجع ما تعذّر حذفه.

    التأجيل إلى ما بعد خروج المزيل كان يشمل كل شيء، فتظهر رسالة «أُزيل
    البرنامج» والمجلد ما زال كاملًا: يفتحه المستخدم فيجد التسعمئة ميغابايت
    مكانها ويظنّ الإزالة فاشلة. والتأجيل يلزم لملف المزيل وحده - فهو يعمل من
    داخل المجلد - لا لبقية الملفات.
    """
    skip = {os.path.normcase(os.path.abspath(path)) for path in keep}
    stuck = []
...
def stop_app(wait=8.0):
    """
    يُغلق البرنامج قبل الإزالة.

    ويندوز يقفل ملفّ أي برنامجٍ يعمل، فحذفُ المجلد وهو مفتوح يترك الـ exe
    ومكتباته - ثلاثةً وأربعين ملفًا في القياس - ثم تستسلم حلقةُ إعادة
    المحاولة بعد عشرين ثانية بصمت، فتبقى بقايا البرنامج إلى الأبد.
    """
    if not app_running():
        return True
    try:
...
def do_uninstall(remove_shortcuts=True):
    ...
    if target and os.path.isdir(target):
        # الآن، لا بعد الخروج: المزيل نفسه وحده يُترك للسكربت المؤجَّل
        wipe(target, keep=[sys.executable])
...
def do_install(target=None, shortcuts=True):
    """
    تثبيتٌ بلا واجهة. يرجع (المجلد، عدد البايتات).

    يلزم للنشر الآليّ، ويلزم لاختبار المثبّت نفسه: بلا وضعٍ صامت لا سبيل
    إلى تجربة التثبيت والإزالة تجربةً حقيقية قبل التسليم.
    """
    target = target or default_target()
    size = extract_payload(target)
    exe = os.path.join(target, EXE_NAME)
    uninstaller = os.path.join(target, "uninstall.exe")
    if not os.path.exists(uninstaller):
        try:
            ...
            uninstaller = ""
    if shortcuts:
        make_shortcut(os.path.join(desktop_dir(), APP_NAME + ".lnk"), exe)
        make_shortcut(os.path.join(start_menu_dir(), APP_NAME + ".lnk"), exe)
        if uninstaller:
            make_shortcut(
                os.path.join(start_menu_dir(), "إزالة %s.lnk" % APP_NAME),
                uninstaller, exe, "--uninstall")
...
def main():
    ...
    if silent:
        where = None
        if "--dir" in sys.argv:
            where = sys.argv[sys.argv.index("--dir") + 1]
        folder, size = do_install(where, "--no-shortcuts" not in sys.argv)
        print(folder)
        print(size)
        return
    run_gui()
...
def demo():
    """فحص ذاتي: المسح يفرغ المجلد الآن ويستثني المزيل وحده."""
    shed = tempfile.mkdtemp(prefix="wun_wipe_")
    try:
        os.makedirs(os.path.join(shed, "_internal", "deep"))
        os.makedirs(os.path.join(shed, "bin"))
        made = []
        for rel in ("app.exe", "uninstall.exe", "bin/ffmpeg.exe",
                    "_internal/base.zip", "_internal/deep/x.pyd"):
            path = os.path.join(shed, rel.replace("/", os.sep))
            with open(path, "wb") as handle:
                handle.write(b"x" * 16)
            made.append(path)
        keeper = os.path.join(shed, "uninstall.exe")

        assert len(made) == 5
        stuck = wipe(shed, keep=[keeper])

        left = []
        for root, _folders, files in os.walk(shed):
            left.extend(os.path.join(root, f) for f in files)
        assert left == [keeper], "بقي غير المزيل: %s" % left
        assert stuck == [keeper], stuck
        # وما عدا المزيل لا يبقى مجلد فارغ يوهم أن شيئًا لم يُحذف
        assert not os.path.isdir(os.path.join(shed, "bin")), "بقي مجلد فارغ"
        print("المسح: حُذف %d ملفًا فورًا · بقي %s ✓"
              % (len(made) - 1, os.path.basename(keeper)))
    finally:
        shutil.rmtree(shed, ignore_errors=True)
    print("build_tools/installer: كل الفحوص سليمة")
