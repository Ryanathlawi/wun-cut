...
def load_fonts() -> str:
    ...
    base = next((f for f in candidates if "thmanyah" in f.lower()), None)
    if base is None:
...
def font(size=10, weight=QFont.Normal, medium=False) -> QFont:
    ...
    الأوجه الثلاثة (Regular / Medium / Bold) تسجَّل كلها تحت اسم عائلة واحد،
    فالوزن يُطلب وزنًا لا باسم عائلة منفصلة. البحث عن عائلة اسمها "…Medium"
    لا يجد شيئًا أبدًا، وكان كل نص "متوسط" في الواجهة يُرسم Regular.
    """
    f = QFont(FONT_FAMILY)
    f.setPointSize(size)
    f.setWeight(QFont.Medium if medium else weight)
    return f


def bold(size=10) -> QFont:
    """عناوين وأرقام: الوجه العريض فعلًا."""
    return font(size, QFont.Bold)
...
#PanelTitle { color: @TXT_DIM; font-size: 9pt; font-weight: 600;
              letter-spacing: 0.5px; }
#Hint       { color: @TXT_MUTE; font-size: 9pt; }
#Value      { color: @TXT_DIM; font-size: 9pt; }

/* الساعة ورموز التوقيت: أرقام عريضة بمقاس ثابت لا ترقص أثناء التشغيل */
#Clock      { color: @TXT; font-size: 11pt; font-weight: 700;
              letter-spacing: 1px; }
...
QPushButton[kind="ghost"] {
    ...
    font-weight: 500; padding: 4px 9px;
}
QPushButton[kind="ghost"]:hover { background: @BG_HOVER; color: @TXT; }

/* رقاقات النظرات الجاهزة: صغيرة ومتساوية، تُقرأ صفًّا لا أزرارًا منفصلة */
QPushButton[kind="chip"] {
    background: @BG_INPUT; border: 1px solid @BORDER;
    border-radius: 13px; padding: 3px 10px;
    color: @TXT_DIM; font-size: 9pt; font-weight: 600;
}
QPushButton[kind="chip"]:hover   { background: @BG_HOVER; color: @TXT;
                                   border-color: @BORDER_HI; }
QPushButton[kind="chip"]:pressed { background: @ACCENT_DEEP; color: #FFFFFF; }

/* ---------- المكتبة ولوحة التأثيرات ---------- */

#ClipName { color: @TXT; font-size: 10pt; font-weight: 700; }
...
#MediaList::item:hover    { background: @BG_HOVER; border-color: @BORDER_HI; }
#MediaList::item:selected { background: @ACCENT_DEEP; border-color: @ACCENT; }

#Timeline { border: 1px solid @BORDER; border-radius: 8px; }
...
#TourTitle { color: @TXT; font-size: 12pt; font-weight: 700; }
#TourBody  { color: @TXT_DIM; font-size: 10pt; }
